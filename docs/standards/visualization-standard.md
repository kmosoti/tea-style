...
- README-level SVGs must render in both GitHub light mode and dark mode.
- Theme-specific README images should use GitHub's `#gh-light-mode-only` and `#gh-dark-mode-only` selectors.
- HTML visualizations should honor `prefers-color-scheme` when practical.
